...
const selectedPair=f=>{const pair=f.safe_bets?.[currentBook()]||(currentBook()===f.bookmaker_provider?f.safe_bet:null);return pair?.available?pair:null;};
const sourceDetails=f=>{const b=marketView(f),p=f.provenance||{};return `<details class="source-details" onclick="event.stopPropagation()" onkeydown="event.stopPropagation()"><summary>Sources &amp; calculation · ${esc(f.summary_id||'Unavailable')}</summary><p>Prices captured: ${esc(b.observed_at||'Unavailable')}</p><p>Match model: ${esc(p.match_engine||f.forecast_engine||'Unavailable')}</p><p>ATS engine: ${esc(p.ats_engine?.engine_id||'Unavailable')}</p><p>${esc(p.prediction_state||'Current projection')}</p>${p.match_fallback_reason?`<p>Fallback: ${esc(p.match_fallback_reason)}</p>`:''}${(b.validation_issues||[]).map(x=>`<p>${esc(x)}</p>`).join('')}</details>`;};
...
const bookmakerProbability=f=>{
  const b=marketView(f),home=Number(b.home_h2h),away=Number(b.away_h2h);
  if(!Number.isFinite(home)||!Number.isFinite(away)||home<=1||away<=1)return {name:b.name,home:null,away:null};
  const rawHome=1/home,rawAway=1/away,total=rawHome+rawAway;
  return {name:b.name,home:rawHome/total,away:rawAway/total};
};
...
function matchCard(f){
  ...
  const fav=favourite(f),safe=selectedPair(f),book=bookmakerProbability(f);
  const bookLabel=book.home==null?'Current H2H prices unavailable':`${esc(f.home_team)} ${pct(book.home)} · ${esc(f.away_team)} ${pct(book.away)}`;
  return `<article class="match-card" data-match="${f.match_id}" tabindex="0"><div class="match-top"><span>${esc(f.kickoff)}</span><span>${esc(f.venue)}</span></div><div class="teams"><div class="team"><b>${esc(f.home_team)}</b><span>${pct(f.home_win_probability)}</span></div><div class="score"><b>${scoreValue(f.likely_home_points)}–${scoreValue(f.likely_away_points)}</b><small>PROJECTED</small></div><div class="team"><b>${esc(f.away_team)}</b><span>${pct(f.away_win_probability)}</span></div></div><div class="prob-label model"><b>NETGAINS MODEL</b><span>${esc(f.home_team)} ${pct(f.home_win_probability)} · ${esc(f.away_team)} ${pct(f.away_win_probability)}</span></div><div class="prob-track model"><i style="width:${finite(f.home_win_probability)?Math.max(0,Math.min(100,f.home_win_probability*100)):0}%"></i><i style="width:${finite(f.away_win_probability)?Math.max(0,Math.min(100,f.away_win_probability*100)):0}%"></i></div><div class="prob-label bookmaker"><b>${esc(book.name)} IMPLIED · MARGIN REMOVED</b><span>${bookLabel}</span></div><div class="prob-track bookmaker"><i style="width:${finite(book.home)?Math.max(0,Math.min(100,book.home*100)):0}%"></i><i style="width:${finite(book.away)?Math.max(0,Math.min(100,book.away*100)):0}%"></i></div><div class="chips"><span class="chip">Favourite <b>${esc(fav.team)}</b></span><span class="chip">Margin <b>${num(f.projected_margin)}</b></span><span class="chip">Total <b>${num(f.projected_total)}</b></span></div>${safe?`<div class="pick"><small>HIGHEST-PROBABILITY TWO-PLAYER COMBINATION</small><b>${safe.legs.map(x=>esc(x.player)).join(' + ')}</b><span>${pct(safe.probability)} joint hit probability · ${esc(safe.method)}</span></div>`:'<div class="empty">Pair calculation unavailable for this bookmaker.</div>'}${sourceDetails(f)}</article>`;
}
...
function render(){
  ...
  renderBoosts();
  $('#modelPanel').innerHTML=fixtures.map(f=>{const p=f.provenance||{},pair=selectedPair(f);return `<section class="engine-status"><h3>${esc(f.home_team)} vs ${esc(f.away_team)}</h3><div class="model-row"><div><b>Match forecasting</b><small>${esc(p.match_engine||f.forecast_engine)}</small></div><b>${p.match_fallback_reason?'FALLBACK':'ACTIVE'}</b></div><div class="model-row"><div><b>ATS engine</b><small>${esc(p.ats_engine?.engine_type||'Unavailable')}</small></div><b>${esc(p.ats_engine?.engine_id||'—')}</b></div><div class="model-row"><div><b>Pair calculation · ${esc(currentBook())}</b><small>${esc(pair?.method||'Unavailable')}${pair?.fallback_reason?' · '+esc(pair.fallback_reason):''}</small></div><b>${pair?.simulation_draws?esc(pair.simulation_draws)+' draws':'—'}</b></div><div class="model-row"><div><b>Evaluation evidence</b><small>${esc(p.calibration_report_status||'No linked evaluation report')}</small></div><b>UNVERIFIED</b></div>${sourceDetails(f)}</section>`;}).join('')||'<div class="empty">No match engine status available.</div>';
