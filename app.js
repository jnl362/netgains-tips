...
function matchCard(f){
  ...
  const fav=favourite(f),safe=f.safe_bet?.available?f.safe_bet:null,book=bookmakerProbability(f);
  ...
  return `<article class="match-card" data-match="${f.match_id}" tabindex="0"><div class="match-top"><span>${esc(f.kickoff)}</span><span>${esc(f.venue)}</span></div><div class="teams"><div class="team"><b>${esc(f.home_team)}</b><span>${pct(f.home_win_probability)}</span></div><div class="score"><b>${scoreValue(f.likely_home_points)}–${scoreValue(f.likely_away_points)}</b><small>PROJECTED</small></div><div class="team"><b>${esc(f.away_team)}</b><span>${pct(f.away_win_probability)}</span></div></div><div class="prob-label model"><b>NETGAINS MODEL</b><span>${esc(f.home_team)} ${pct(f.home_win_probability)} · ${esc(f.away_team)} ${pct(f.away_win_probability)}</span></div><div class="prob-track model"><i style="width:${finite(f.home_win_probability)?Math.max(0,Math.min(100,f.home_win_probability*100)):0}%"></i><i style="width:${finite(f.away_win_probability)?Math.max(0,Math.min(100,f.away_win_probability*100)):0}%"></i></div><div class="prob-label bookmaker"><b>${esc(book.name)} IMPLIED · MARGIN REMOVED</b><span>${bookLabel}</span></div><div class="prob-track bookmaker"><i style="width:${finite(book.home)?Math.max(0,Math.min(100,book.home*100)):0}%"></i><i style="width:${finite(book.away)?Math.max(0,Math.min(100,book.away*100)):0}%"></i></div><div class="chips"><span class="chip">Favourite <b>${esc(fav.team)}</b></span><span class="chip">Margin <b>${num(f.projected_margin)}</b></span><span class="chip">Total <b>${num(f.projected_total)}</b></span></div>${safe?`<div class="pick"><small>SAFEST ATS COMBINATION</small><b>${safe.legs.map(x=>esc(x.player)).join(' + ')}</b><span>${pct(safe.probability)} joint hit probability</span></div>`:''}</article>`;
}
